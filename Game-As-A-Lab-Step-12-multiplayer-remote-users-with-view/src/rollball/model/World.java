...
import rollball.model.ser.WorldSer;
...
public class World {
	...
	public void updateState(WorldSer ser) {
		picks.clear();
		for (GameObjectSer obj: ser.getPickableObj()) {
			picks.add(GameFactory.getInstance().createPickUpObject(obj));
		}
		balls.clear();
		for (GameObjectSer obj: ser.getBalls()) {
			balls.add(GameFactory.getInstance().createBall(obj));
		}
	}

	public WorldSer serialize() {
		List<GameObjectSer> picksSer = new ArrayList<GameObjectSer>();
		for (GameObject obj: picks) {
			picksSer.add(obj.serialize());
		}
		List<GameObjectSer> ballsSer = new ArrayList<GameObjectSer>();
		for (GameObject obj: balls) {
			ballsSer.add(obj.serialize());
		}
		return new WorldSer(picksSer,ballsSer);
	}
